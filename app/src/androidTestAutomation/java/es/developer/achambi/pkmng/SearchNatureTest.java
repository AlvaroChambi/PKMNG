...
public class SearchNatureTest extends BaseAutomationTest {
    @Test
    public void abilitySearchTest() {
        ...
        onView( withId(R.id.configuration_nature_frame) )
                .perform(scrollTo(), click());
        onView( allOf( withId( R.id.nature_name_text ),
                withText( "modest" ),
                isDescendantOfA( withId( R.id.base_search_header_frame ) ) ) )
                .check( matches( isDisplayed() ) );

        onView(withId(R.id.overview_search_action)).perform(click());
        onView(withId(R.id.search_src_text)).perform( clearText(), typeText("lonely"),
                pressImeActionButton() );
        delay(500);// don't know why :(
        onView( allOf( withId(R.id.nature_name_text), withText("lonely") ) )
                .check(matches(isDisplayed()));
        ...
        onView( allOf( withId(R.id.nature_name_text),
                withText("hardy") ) ).check(matches( isDisplayed() ));
    }
}
